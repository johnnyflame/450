...
def procedural_material(name, colour):
    ...
    #Texture type is randomly selected out of a list
...
def break_temple (xSize,ySize,height):
    dimensions = (xSize,ySize,height)
    crack = bpy.ops.mesh.primitive_plane_add
    
    crack(location = (xSize*0.5,ySize*0.5,0))
...
def temple():
    ...
    if bool(random.getrandbits(1)):
        green = red + red * random.uniform(0,0.15)
    else:
        green = red - red * random.uniform(0,0.15)
        
    if bool(random.getrandbits(1)):   
        blue = green + green * random.uniform(0,0.15)
    else:
        blue = green - green * random.uniform(0,0.15)
