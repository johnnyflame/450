   ...
   #Make roof
    ...
    for s in range(0,columns_per_longedge):
        ...
        if use_library:
            ...
            , base_z_location ,height=column_base_height+column_pillar_height,b_width=column_base_size)
    ...
    for s in range (0, columns_per_shortedge):
        ...
        bpy.ops.mesh.primitive_cube_add(location = (podium_x + (podium_width - column_base_size - column_base_edge) - s * (2 * (podium_width - column_base_size - column_base_edge)/(columns_per_shortedge -1)), podium_y + podium_length /2.0, 2 * podium_height + column_base_height))                 
        bpy.ops.transform.resize(value = (column_base_size,column_base_size,column_base_height))
        ...
        bpy.ops.mesh.primitive_cube_add(location = (podium_x + (podium_width - column_base_size - column_base_edge) - s * (2 * (podium_width - column_base_size - column_base_edge)/(columns_per_shortedge -1)), podium_y - (podium_length - column_base_size -column_base_edge), 2 * podium_height + column_base_height))               
        bpy.ops.transform.resize(value = (column_base_size,column_base_size,column_base_height))
...
def stylized_pillars(locX,locY,locZ,height,b_width):
    ...
    bpy.ops.mesh.add_column(Style="0",col_base=True,addendum=0.04,col_plinth=False,base_type=7,col_taper=0.03,col_flutes=8,col_blocks=1,base_width=b_width,col_radius=b_width,cap_width=b_width)
